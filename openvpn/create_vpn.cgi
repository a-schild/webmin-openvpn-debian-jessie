...
#    close(CMD);
	...
	print &ui_table_row($text{'name'}, $in{'VPN_NAME'});
	...
	$dev = $1;
	$numberdev = $2;
	print &ui_table_row($text{'dev'}, &ui_hidden('dev',$in{'dev'}).$dev." ".&ui_textbox('numberdev', $numberdev, 6));
    } else {
	print &ui_table_row($text{'dev'}, &ui_select('dev', $in{'dev'}, [ ['tun','tun'],['tap','tap'] ]));
    }
...
##############################
    ...
    print &ui_table_row($text{'ca'}, $in{'ca_name'});
    if (@{$a_server}) { 
