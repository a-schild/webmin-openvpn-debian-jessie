...
if (@$a_clients) {
    ...
    print &ui_table_start($text{'new_client_title'}.' '.$in{'vpn'},'width=100%');
    print &ui_table_row($text{'name'}, &ui_select('CLIENT_NAME', '', $a_clients),'',[ 'width="50%"' ])."</tr>\n";
    ...
    print "<tr>".&ui_table_row($text{'remote'}, $text{'remote_url'}.': '.&ui_textbox('remote_url',$config{'default_server'},12).' '.$text{'remote_port'}.': '.$in{'port'},'',[ 'width="50%"' ])."</tr>\n";
    if ($in{'tls-auth'} == 1) {
    ...
    print "<tr>".&ui_table_row($text{'float'}, &ui_select('float', 1, [ ['0',$text{'no'}],['1',$text{'yes'} ] ]),'',[ 'width="50%"' ])."</tr>\n";
    ...
    print "<tr>".&ui_table_row($text{'ccdfile-content'}, &ui_textarea('ccdfile', '', 3, 45, 'off'),'',[ 'width="50%"' ])."</tr>\n";
    print &ui_table_end();
